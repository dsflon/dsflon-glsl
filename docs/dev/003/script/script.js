...
(() => {
    ...
    window.addEventListener('load', () => {
        ...
        canvas.addEventListener('touchstart', camera.touchStartEvent, false);
        canvas.addEventListener('touchmove', camera.touchMoveEvent, false);
        canvas.addEventListener('touchend', camera.touchEndEvent, false);
        ...
        createTexture('./image/p.png', (texture) => {
    ...
    function init(texture){
        ...
        //time
        scenePrg.uniLocation[4] = gl.getUniformLocation(scenePrg.program, 'random');
        scenePrg.uniType[4]     = 'uniform1f';

        //texture
        scenePrg.uniLocation[5] = gl.getUniformLocation(scenePrg.program, 'textureUnit');
        ...
        let texCoord = [];
        {
            ...
            let interval = 0.005; // 頂点同士の間隔
            let count = width / interval; // 幅と間隔からループカウント数を求める
            // ループで一気に頂点を定義する

            for ( var i = 0, j = count; i < j; i ++ ) {

                var phi = Math.acos( -half + ( width * i ) / j );
                var theta = Math.sqrt(j * Math.PI ) * phi;
                ...
                position.push(x, y, z);
                color.push(Math.random(), Math.random(), 1.0, 1.0);
                // texCoord.push(i / count, i / count);
                texCoord.push(x, y, z);
            ...
            //
            //         position.push(x, y, z);
            //         color.push(i / count, j / count, 1.0, 1.0);
            //         texCoord.push(i / count, j / count);
            //
            //     }
            // }
        }
        ...
        // WebGL API 関連の初期設定
        gl.clearColor(0.95, 0.95, 0.95, 1.0); // クリアする色
        ...
        function render(){
            ...
            // fovy = field of view (y)角度
            let fovy   = 75 * camera.scale;          // カメラの視野角
            let aspect = canvasWidth / canvasHeight; // カメラのアスペクト比
            let near   = 0.1;                        // 最近距離クリップ面
            let far    = 1000.0;                       // 最遠距離クリップ面
            ...
            mat.rotate(mMatrix, nowTime * 0.05, [0.5, 1.0, 0.0], mMatrix);
            // mat.rotate(mMatrix, -0.4, [1.0, 0.0, 0.0], mMatrix);
            ...
            gl[scenePrg.uniType[4]](scenePrg.uniLocation[4], Math.random());
            gl[scenePrg.uniType[5]](scenePrg.uniLocation[5], 0);
    ...
    class InteractionCamera {
        ...
        touchStartEvent(eve){
            this.dragging = true;
            this.prevTouch = [eve.touches[0].clientX, eve.touches[0].clientY];
        }
        /**
         * mouse move event
         * @param {Event} eve - event object
        ...
        touchMoveEvent(eve){
            if(this.dragging !== true){return;}
            let x = this.prevTouch[0] - eve.touches[0].clientX;
            let y = this.prevTouch[1] - eve.touches[0].clientY;
            this.rotation = Math.sqrt(x * x + y * y) / this.rotationScale * this.rotatePower;
            this.rotateAxis[0] = y;
            this.rotateAxis[1] = x;
            this.prevTouch = [eve.touches[0].clientX, eve.touches[0].clientY];
