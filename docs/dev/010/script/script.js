...
(() => {
    ...
    window.addEventListener('load', () => {
        ...
        // マウス関連イベントの登録
            ...
            window.addEventListener("deviceorientation", function(event) {
                // if( -90 < event.gamma && event.gamma < 90 ) gamma = event.gamma / 90;
                // if( -90 < event.beta && event.beta < 90 ) beta = event.beta / 90;
                // if( -85 < event.beta && event.beta < 85 )  {
                ...
                // document.getElementById("num").innerHTML = "gamma: " + gamma + "<br> beta: " + beta;
            }, false);
        ...
        // ************************************************
        let loadCount = 0;
        for (var i = 0; i < images.length; i++) loadImg(i);
        function loadImg(i) {
            let img = new Image();
            img.src = images[i];
            img.onload = function() {
                loadCount ++;
                imagesElm.splice(i, 0, img);
                if( loadCount == images.length ) {

                    createTextures(imagesElm[0], (texture, imgSize) => {
        ...
        // createTexture(images[0], (texture, imgSize) => {
        //     gl.activeTexture(gl.TEXTURE0);
        //     gl.bindTexture(gl.TEXTURE_2D, texture);
        //     DrawShader();
        //
    ...
    function init(texture){
        ...
        scenePrg.uniLocation[9] = gl.getUniformLocation(scenePrg.program, 'mousePrev');
        scenePrg.uniType[9]     = 'uniform2fv';
        ...
        //deviceorientation
        scenePrg.uniLocation[7] = gl.getUniformLocation(scenePrg.program, 'deviceorientation');
        scenePrg.uniType[7]     = 'uniform2fv';

        //deviceorientation
        scenePrg.uniLocation[8] = gl.getUniformLocation(scenePrg.program, 'slideTimer');
        ...
        function render(){
            ...
            if(usermedia) updateTextureMedia();
            // フレームバッファに対する描画はここまで -------------------------
            ...
            gl[scenePrg.uniType[9]](scenePrg.uniLocation[9], mousePrev);
            gl[scenePrg.uniType[6]](scenePrg.uniLocation[6], isSp);
            gl[scenePrg.uniType[7]](scenePrg.uniLocation[7], deviceorientation);
            gl[scenePrg.uniType[8]](scenePrg.uniLocation[8], slideTimer);
    ...
    function SlideImg() {
        ...
        createTextures(imagesElm[slideCount % imagesLength], (texture,imgSize) => {
            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, texture);
            imgAspect = imgSize.aspect;
            imgWidth = imgSize.imgWidth;
    ...
    function createTextures(img, callback){
        let tex = gl.createTexture();
        gl.bindTexture(gl.TEXTURE_2D, tex);
        // gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true); // 上下反転
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, img);
