...
(() => {
    ...
    function init(texture){
        ...
        //texture
        scenePrg.uniLocation[4] = gl.getUniformLocation(scenePrg.program, 'textureUnit');
        ...
        let texCoord = [];
        {
            ...
            const AMOUNTX = fontSize * 2;
            const AMOUNTY = fontSize;
            const SEPARATION = 14;

            let ctx = CreateText(text,fontSize,fontFamily);

            const AMOUNT = AMOUNTY + fontSize + ctx.lineHeight;
        	const data = ctx.context.getImageData(0, 0, AMOUNTX, AMOUNT).data;
            ...
            //         let y = -half + j;
            //
            //         position.push(x, y, 1.0);
            //         color.push(i / count, j / count, 1.0, 1.0);
            //         texCoord.push(i / count, j / count, 1.0);
            //
            //     }
            // }
            var i = 0;
    		for ( var ix = 0; ix < AMOUNTX; ix ++ ) {
    			for ( var iy = 0; iy < AMOUNTY; iy ++ ) {
    				...
    				if( alpha != 0 && style == "rgb(255, 255, 255)" ) {
                        ...
                        color.push(ix / AMOUNTX, iy / AMOUNTY, 1.0, 1.0);
                        texCoord.push(ix / AMOUNTX, iy / AMOUNTY, 1.0);
        ...
        function render(){
            ...
            gl[scenePrg.uniType[4]](scenePrg.uniLocation[4], 0);
